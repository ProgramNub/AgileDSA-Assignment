...
public class BouquetOrder {
    ...
    public static ListInterface<ItemEnjiun> Bouquet(ListInterface<ItemEnjiun> items, ListInterface<ProductEnjiun> products) {
        ...
        do {
            ...
            bouquetTtlPrice = bouquet.calculateBill(products.get(Integer.parseInt(n) - 1).getPrice(), Integer.parseInt(qty));
            products.get(Integer.parseInt(n) - 1).setQuantity(products.get(Integer.parseInt(n) - 1).getQuantity() - Integer.parseInt(qty));

            //Add ordered item
            ItemEnjiun item = new ItemEnjiun(products.get(Integer.parseInt(n) - 1).getProductID(), products.get(Integer.parseInt(n) - 1).getName(), 'B', bouquetTtlPrice, Integer.parseInt(qty));
            items.add(item);
