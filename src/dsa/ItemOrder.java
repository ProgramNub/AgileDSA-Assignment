...
public class ItemOrder {
    ...
    public static boolean Ordering(ListInterface<Product> products, ListInterface<Item> items, String bp, Customer customer, ListInterface<Corporate> corporates, ListInterface<Order> orders, ListInterface<Consumer> consumers, Boolean paid, Boolean checked, String orderID, ListInterface<Invoice> invoice) {
                    ...
                    if (checked == false) {
                        do {
                            ...
                            if (!credExceed.matches("^[OoSsCc]$")) {
                                ...
                                ItemOrder.salesOrder(products, corporates, consumers, items, orders, true, customer, orderID, invoice);
                            } else if (credExceed.matches("^[Cc]$")) {
                                MainMenu.main(products, corporates, consumers, items, orders, invoice);
                            }
                        } while (!credExceed.matches("^[IiCc" + t + Character.toLowerCase(t) + "]$"));
                    }
                } else {
                    paid = item.corpCredit(paid, corp, productTtlPrice);
                }
            }

            //Add ordered item
            item = new Item(tempList.get(Integer.parseInt(n) - 1).getProductID(), tempList.get(Integer.parseInt(n) - 1).getProductName(), t, productTtlPrice, Integer.parseInt(qty), orderID);
    ...
    public static void salesOrder(ListInterface<Product> products, ListInterface<Corporate> corporates, ListInterface<Consumer> consumers, ListInterface<Item> items, ListInterface<Order> orders, boolean paid, Customer customer, String orderID, ListInterface<Invoice> invoice) {
        Scanner scan = new Scanner(System.in);
        //Display order
        ...
        Order order = new Order();
        boolean add = order.addOrder(items, orders, customer, paid, delPick, hour, orderTtlPrice, dNow, receiveDate, orderID);
